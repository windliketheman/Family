...
- (void)addSubviews
{
    ...
    self.pageControl.pageIndicatorTintColor = [UIColor colorWithRed:255.0f green:255.0f blue:255.0f alpha:0.3];
...
- (void)reloadPages
{
    ...
    [[self scrollViewPages] enumerateObjectsUsingBlock:^(UIView *obj, NSUInteger idx, BOOL *stop) {
        ...
        obj.contentMode = UIViewContentModeScaleToFill;
...
{
    CGRect scrollViewRect = self.pagingScrollView.frame;
    ...
    [UIView animateWithDuration:0.3f animations:^{
        if (wself)
        {
            ...
            if (!object) [self.pagingScrollView setFrame:scrollViewRect];
            [self.pagingScrollView setAlpha:0.0f];
        }
